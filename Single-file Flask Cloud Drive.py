...
Dependencies:
    - flask: Web server, routes, sessions, and template rendering.
    - py7zr: Creates .7z archives.
    - charset-normalizer: Helps detect text file encodings.
Install:
    pip install flask py7zr charset-normalizer
Run:
    python app.py
Optional environment variables:
    SECRET_KEY  # Fixed session secret; generate with: python -c 'import secrets; print(secrets.token_hex())'
    DRIVE_DATA_DIR
    MAX_CONTENT_LENGTH
...
DATA_DIR = Path(os.environ.get("DRIVE_DATA_DIR", BASE_DIR / "data"))  # Data root directory; can be overridden by an environment variable
UPLOAD_DIR = DATA_DIR / "uploads"  # Directory for uploaded source files
...
TMP_ARCHIVE_DIR.mkdir(parents=True, exist_ok=True)  # Create the temporary archive directory on first startup

def load_secret_key() -> str:
    """Load a stable Flask secret key from env or a local data file.

    Deployment note:
        You can generate a fixed key once and set it as SECRET_KEY:
        python -c 'import secrets; print(secrets.token_hex())'

    Keeping SECRET_KEY stable avoids invalidating existing signed sessions after restart.
    """
    # Prefer an operator-provided key so production deployments can stay fixed.
    ...
    # Local fallback: create one stable key on first run and reuse it later.
    generated_secret_key = os.urandom(32).hex()
    secret_key_path.write_text(generated_secret_key, encoding="utf-8")
    try:
        # Best effort only: chmod may not be supported on every platform.
...
app = Flask(__name__, static_folder=None)  # Static assets are served by embedded routes
app.secret_key = load_secret_key()  # Used to sign Flask sessions
app.config["MAX_CONTENT_LENGTH"] = int(os.environ.get("MAX_CONTENT_LENGTH", 1024 * 1024 * 1024))  # Default maximum upload size: 1 GB
app.config.update(
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">Log in</button>
    </form>
    <div class="auth-links"><span></span><a href="{{ url_for('register') }}">Register</a></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">Register</button>
    </form>
    <div class="auth-links"><a href="{{ url_for('login') }}">Log in</a><span></span></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">Save</button>
    </form>
    <div class="auth-links"><a href="{{ url_for('drive') }}">Back</a><a href="{{ url_for('logout') }}">Log out</a></div>
  </div>
</section>
{% endblock %}
""",
...
<div class="drive-shell" data-current-folder-id="{{ current_folder_id or '' }}">
  ...
  <section id="dropArea" class="file-board" aria-label="File List">
      ...
      <div class="entry-card {{ e.kind }}-entry" draggable="true" data-id="{{ e.id }}" data-kind="{{ e.kind }}" data-name="{{ e.name }}" data-media-kind="{{ media_kind(e.name) or '' }}" data-is-text="{{ '1' if is_text_file(e.name) else '0' }}">
        ...
        <div class="entry-main">
          ...
          <div class="entry-meta">
            {% if e.kind == 'folder' %}
              <span class="kind-badge folder-badge">Folder</span>
            {% else %}
    ...
    {% else %}
      <div class="empty-hint">Right-click empty space.</div>
    {% endfor %}
  </section>
...
{% block title %}Play Online - {{ item.name }}{% endblock %}
{% block body %}
...
<section class="media-shell share-single-file" data-share-token="{{ token }}">
  ...
  <section id="sharedFileBoard" class="file-board single-shared-file-board" aria-label="Shared File">
...
<div class="drive-shell shared" data-share-token="{{ token }}">
  <header class="drive-header">
    ...
    <div class="drive-nav">
      <a class="mini-download" data-shared-archive-link="1" data-prepare-url="{{ url_for('share_prepare_archive_download', token=token, folder_id=current_folder_id) }}" href="{{ url_for('share_archive_download', token=token, folder_id=current_folder_id) }}">Download All .7z</a>
    </div>
  </header>

  <section id="sharedFileBoard" class="file-board shared-board" aria-label="Shared File List">
    {% for e in entries %}
...
{% block title %}Share Management{% endblock %}
{% block body %}
<div class="shares-shell">
  <header class="drive-header shares-header">
    <div class="drive-title-area">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          <li class="breadcrumb-item"><a href="{{ url_for('drive') }}">Files</a></li>
          <li class="breadcrumb-item active"><span>Share Management</span></li>
        </ol>
      </nav>
    </div>
    <div class="drive-nav">
      <a class="nav-pill" href="{{ url_for('change_password') }}">Change Password</a>
      <a class="nav-pill" href="{{ url_for('logout') }}">Log out</a>
    </div>
  </header>
      ...
      <article class="share-row" data-share-id="{{ s.id }}" data-share-url="{{ share_url }}">
        <div class="share-row-main">
          ...
          <div class="share-info">
            ...
            <div class="entry-meta">
              <span class="kind-badge {{ 'folder-badge' if s.kind == 'folder' else 'file-badge' }}">{{ 'Folder' if s.kind == 'folder' else kind_label(s.kind, s.name) }}</span>
        ...
        <div class="share-actions">
          <a class="ghost-action" href="{{ share_url }}" target="_blank" rel="noopener">Open</a>
          <button class="ghost-action copy-share" type="button">Copy</button>
          <button class="ghost-action danger cancel-share" type="button">Cancel Share</button>
        </div>
      </article>
    {% else %}
      <div class="empty-hint share-empty">No shares.</div>
    {% endfor %}
  </section>
  <div id="toastHost" class="toast-host"></div>
</div>
...
{% block title %}{% if read_only %}View{% else %}Edit{% endif %} - {{ item.name }}{% endblock %}
{% block body %}
<section class="text-editor-shell" data-save-url="{{ save_url or '' }}" data-read-only="{{ '1' if read_only else '0' }}">
  <header class="text-editor-header">
    ...
    <div class="text-editor-actions">
      <span id="textEditorStatus" class="text-editor-status">{% if read_only %}Read-only{% else %}Loaded{% endif %}</span>
      <a class="text-editor-link" href="{{ download_url }}">Download</a>
      {% if not read_only %}
        <button id="saveTextButton" class="text-editor-button" type="button">Save</button>
      {% endif %}
    </div>
  </header>
...
// Folder archives can take a while; keep the page patient until Flask returns the ready file URL.
...
async function prepareArchiveDownload(prepareUrl) {
  const overlay = showArchiveWaitingOverlay('Compressing, please wait...');
  try {
...
function openEntryMenu(entry, x, y) {
  ...
  addMenuItem('Rename', async () => {
    const next = prompt('Enter a new name:', name);
    if (!next || next.trim() === name) return;
    await api('/api/rename', { item_id: id, name: next.trim() });
    location.reload();
  });
  addMenuItem('Share', async () => {
    const data = await api('/api/share', { item_id: id });
    const copied = await copyText(data.url);
    if (copied) {
      showToast('Share link copied to clipboard.');
    } else {
      showToast(`Copy failed. Please copy manually: ${data.url}`, 8000);
      prompt('Copy share link:', data.url);
    }
  });
...
function openBlankMenu(x, y) {
  ...
  addMenuItem('New Folder', async () => {
    const name = prompt('Folder name:', 'New Folder');
    if (!name) return;
    await api('/api/folders', { parent_id: currentFolderId, name: name.trim() });
    location.reload();
  });
  addMenuItem('Share Management', () => { window.location.href = '/shares'; });
  addMenuItem('Change Password', () => { window.location.href = '/change-password'; });
  addMenuItem('Log out', () => { window.location.href = '/logout'; });
  placeMenu(x, y);
}
...
async function prepareSharedArchiveDownload(prepareUrl) {
  const overlay = showSharedArchiveWaitingOverlay('Compressing, please wait...');
  try {
    const response = await fetch(prepareUrl, {
      headers: { 'Accept': 'application/json' },
      credentials: 'same-origin'
...
function openSharedEntryMenu(entryElement, clientX, clientY) {
  ...
  if (entryElement.dataset.kind === 'folder') {
    ...
    addSharedMenuItem('Download as 7z', () => prepareSharedArchiveDownload(`/api/s/${shareToken}/prepare_archive/${entryElement.dataset.id}`));
  } else {
...
document.querySelectorAll('.share-row').forEach(row => {
  row.querySelector('.copy-share')?.addEventListener('click', async () => {
    ...
    else prompt('Copy share link:', url);
  });

  row.querySelector('.cancel-share')?.addEventListener('click', async () => {
    const name = row.querySelector('.entry-name')?.textContent || 'this share';
    if (!confirm(`Cancel sharing for "${name}"?`)) return;
    const res = await fetch(`/api/shares/${row.dataset.shareId}/cancel`, { method: 'POST' });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.ok === false) {
      showToast(data.error || 'Failed to cancel share.', 4200);
      return;
    }
    row.remove();
...
async function saveTextDocument() {
  ...
  try {
    ...
    if (!response.ok || responseData.ok === false) throw new Error(responseData.error || 'Save failed');
    hasUnsavedTextChanges = false;
    setEditorStatus(`Saved: ${new Date().toLocaleString()}`);
  } catch (error) {
...
if (textContentEditor) {
  textContentEditor.addEventListener('input', () => {
    ...
    setEditorStatus('Unsaved changes. Press Ctrl/⌘ + S to save.');
  });
...
def add_security_headers(response):
    """Add conservative browser security headers for the single-file app."""
...
def init_db():
    ...
    db.executescript(
        """
        -- Users table: stores username, password salt, password hash, and creation time.
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- User primary key, auto-increment integer.
            username TEXT NOT NULL UNIQUE,        -- Login username; required and globally unique.
            password_salt TEXT NOT NULL,          -- Per-user random salt used in password hashing.
            password_hash TEXT NOT NULL,          -- SHA3 hash calculated from salt + password.
            created_at TEXT NOT NULL              -- User creation time stored as an ISO string.
        );

        -- Entries table: files and folders share one table and are distinguished by kind.
        CREATE TABLE IF NOT EXISTS entries (
            id INTEGER PRIMARY KEY AUTOINCREMENT,                 -- Entry primary key, auto-increment integer.
            owner_id INTEGER NOT NULL,                            -- Owner user ID; references users.id.
            parent_id INTEGER NULL,                               -- Parent folder ID; NULL means the user root folder.
            kind TEXT NOT NULL CHECK(kind IN ('file', 'folder')), -- Entry type; only file or folder is allowed.
            name TEXT NOT NULL,                                   -- File or folder name shown to the user.
            storage_name TEXT NULL,                               -- Actual on-disk storage filename; folders are usually NULL.
            size INTEGER NOT NULL DEFAULT 0,                      -- File size in bytes; folders default to 0.
            created_at TEXT NOT NULL,                             -- Entry creation time stored as an ISO string.
            updated_at TEXT NOT NULL,                             -- Entry update time; refreshed by rename/move operations.
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE,   -- Cascade-delete entries when the user is deleted.
            FOREIGN KEY(parent_id) REFERENCES entries(id) ON DELETE CASCADE -- Cascade-delete child entries when the parent folder is deleted.
        );

        -- Root-name uniqueness: a user cannot have two root entries with the same name.
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_root
            ON entries(owner_id, name)
            WHERE parent_id IS NULL;

        -- Folder-name uniqueness: entries under the same parent cannot share a name.
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_folder
            ON entries(owner_id, parent_id, name)
            WHERE parent_id IS NOT NULL;

        -- Shares table: maps share tokens to shared files or folders.
        CREATE TABLE IF NOT EXISTS shares (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- Share record primary key, auto-increment integer.
            token TEXT NOT NULL UNIQUE,           -- Random token used in public share links; must be unique.
            item_id INTEGER NOT NULL,             -- Shared file or folder ID; references entries.id.
            owner_id INTEGER NOT NULL,            -- Share creator ID; references users.id.
            created_at TEXT NOT NULL,             -- Share creation time stored as an ISO string.
            expires_at TEXT NULL,                 -- Share expiration time; NULL means no expiration is currently set.
            FOREIGN KEY(item_id) REFERENCES entries(id) ON DELETE CASCADE, -- Cascade-delete shares when the entry is deleted.
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE  -- Cascade-delete shares when the user is deleted.
        );
        """
    )
    db.commit()
...
def register():
    if request.method == "POST":
        ...
        # Keep password policy intentionally loose: only reject empty values.
...
def change_password():
    ...
    if request.method == "POST":
        ...
        # Same loose rule here: a new password only needs to be non-empty.
...
def unique_name(owner_id: int, parent_id: int | None, desired: str, exclude_id: int | None = None) -> str:
    ...
    for i in range(1, 10000):
        ...
        if not exists(candidate):
            return candidate
    raise RuntimeError("Unable to generate a unique name")
...
def api_create_folder():
    ...
    name = clean_segment(data.get("name", "New Folder"))
...
def api_cancel_share(share_id):
    ...
    get_db().commit()
    if cur.rowcount == 0:
        return jsonify(ok=False, error="Share does not exist or you do not have permission."), 404
    return jsonify(ok=True)
