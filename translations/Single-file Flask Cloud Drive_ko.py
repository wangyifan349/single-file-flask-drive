...
# -*- coding: utf-8 -*-
"""
단일 파일 Flask 클라우드 드라이브
=============================================
개요:
    이 파일은 단일 파일 배포 버전입니다. 백엔드 라우트, HTML 템플릿,
    CSS 및 JavaScript가 app.py 안에 포함되어 있습니다. 배포하려면 이 파일을
    복사하고 필요한 의존성을 설치하세요.
주요 기능:
    - 로그인, 회원가입, 로그아웃 및 비밀번호 변경.
    - 무작위 솔트와 SHA3-256으로 저장되는 비밀번호 다이제스트.
    - 계층형 폴더, 파일/폴더 업로드, 목록 표시, 다운로드, 이름 변경,
      삭제 및 드래그 앤 드롭 이동 지원.
    - 파일/폴더 공유, 공유 관리 및 공유 취소.
    - 공유 폴더 탐색 가능. 폴더 다운로드 시 전체 아카이브를 메모리에
      올리지 않고 임시 .7z 파일을 생성합니다.
    - 오디오/비디오 온라인 재생.
    - 일반 텍스트 온라인 편집. 공유 페이지는 읽기 전용 텍스트 보기를 지원합니다.
    - 텍스트 읽기는 BOM과 일반적인 인코딩을 감지하며, 저장 시 원래 인코딩과
      줄바꿈 스타일을 보존하려고 시도합니다.
의존성:
    - flask: 웹 서버, 라우트, 세션 및 템플릿 렌더링.
    - py7zr: .7z 아카이브 생성.
    - charset-normalizer: 텍스트 파일 인코딩 감지 지원.
설치:
    pip install flask py7zr charset-normalizer
실행:
    python app.py
선택적 환경 변수:
    SECRET_KEY  # 고정 세션 비밀 키. 생성 예: python -c 'import secrets; print(secrets.token_hex())'
    DRIVE_DATA_DIR
    MAX_CONTENT_LENGTH
...
except ImportError:  # pragma: no cover - 의존성이 없을 때 런타임 대체 처리
    detect_character_sets = None


BASE_DIR = Path(__file__).resolve().parent  # app.py가 들어 있는 디렉터리
DATA_DIR = Path(os.environ.get("DRIVE_DATA_DIR", BASE_DIR / "data"))  # 데이터 루트 디렉터리. 환경 변수로 재정의할 수 있음
UPLOAD_DIR = DATA_DIR / "uploads"  # 업로드된 원본 파일 디렉터리
TMP_ARCHIVE_DIR = DATA_DIR / "tmp_archives"  # 임시 .7z 아카이브 디렉터리
DB_PATH = DATA_DIR / "drive.db"  # SQLite 데이터베이스 파일
ARCHIVE_RETENTION_SECONDS = int(os.environ.get("ARCHIVE_RETENTION_SECONDS", 24 * 60 * 60))  # 임시 아카이브 보존 시간

DATA_DIR.mkdir(parents=True, exist_ok=True)  # 최초 시작 시 데이터 디렉터리 생성
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)  # 최초 시작 시 업로드 디렉터리 생성
TMP_ARCHIVE_DIR.mkdir(parents=True, exist_ok=True)  # 최초 시작 시 임시 아카이브 디렉터리 생성

def load_secret_key() -> str:
    """환경 변수 또는 로컬 데이터 파일에서 안정적인 Flask 비밀 키를 로드합니다.

    배포 참고:
        고정 키를 한 번 생성하고 SECRET_KEY로 설정할 수 있습니다:
        python -c 'import secrets; print(secrets.token_hex())'

    SECRET_KEY를 안정적으로 유지하면 재시작 후 기존 서명된 세션이 무효화되는 것을 방지할 수 있습니다.
    """
    # 운영자가 제공한 키를 우선 사용하여 운영 배포가 고정된 상태를 유지하도록 합니다.
...
app = Flask(__name__, static_folder=None)  # 정적 자산은 내장 라우트로 제공됩니다
app.secret_key = load_secret_key()  # Flask 세션 서명에 사용됩니다
app.config["MAX_CONTENT_LENGTH"] = int(os.environ.get("MAX_CONTENT_LENGTH", 1024 * 1024 * 1024))  # 기본 최대 업로드 크기: 1GB
app.config.update(
...
<head>
  ...
  <title>{% block title %}클라우드 드라이브{% endblock %}</title>
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">로그인</button>
    </form>
    <div class="auth-links"><span></span><a href="{{ url_for('register') }}">회원가입</a></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">회원가입</button>
    </form>
    <div class="auth-links"><a href="{{ url_for('login') }}">로그인</a><span></span></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <label class="form-label mt-4">새 비밀번호 확인</label>
      <input class="form-control form-control-lg" name="confirm_password" type="password" autocomplete="new-password" required>
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">저장</button>
    </form>
    <div class="auth-links"><a href="{{ url_for('drive') }}">뒤로</a><a href="{{ url_for('logout') }}">로그아웃</a></div>
  </div>
</section>
{% endblock %}
""",
...
<div class="drive-shell" data-current-folder-id="{{ current_folder_id or '' }}">
  ...
  <section id="dropArea" class="file-board" aria-label="파일 목록">
    {% for e in entries %}
      <div class="entry-card {{ e.kind }}-entry" draggable="true" data-id="{{ e.id }}" data-kind="{{ e.kind }}" data-name="{{ e.name }}" data-media-kind="{{ media_kind(e.name) or '' }}" data-is-text="{{ '1' if is_text_file(e.name) else '0' }}">
        ...
        <div class="entry-main">
          ...
          <div class="entry-meta">
            {% if e.kind == 'folder' %}
              <span class="kind-badge folder-badge">폴더</span>
            {% else %}
    ...
    {% else %}
      <div class="empty-hint">빈 공간을 마우스 오른쪽 버튼으로 클릭하세요.</div>
    {% endfor %}
  </section>
...
{% block title %}온라인 재생 - {{ item.name }}{% endblock %}
{% block body %}
<section class="media-shell">
  <header class="media-header">
    ...
    <a class="mini-download" href="{{ download_url }}">다운로드</a>
  </header>
...
<section class="media-shell share-single-file" data-share-token="{{ token }}">
  ...
  <section id="sharedFileBoard" class="file-board single-shared-file-board" aria-label="공유 파일">
...
<div class="drive-shell shared" data-share-token="{{ token }}">
  <header class="drive-header">
    ...
    <div class="drive-nav">
      <a class="mini-download" data-shared-archive-link="1" data-prepare-url="{{ url_for('share_prepare_archive_download', token=token, folder_id=current_folder_id) }}" href="{{ url_for('share_archive_download', token=token, folder_id=current_folder_id) }}">모두 .7z로 다운로드</a>
    </div>
  </header>

  <section id="sharedFileBoard" class="file-board shared-board" aria-label="공유 파일 목록">
    {% for e in entries %}
    ...
    {% else %}
      <div class="empty-hint">비어 있습니다.</div>
    {% endfor %}
  </section>
...
<div class="shares-shell">
  <header class="drive-header shares-header">
    <div class="drive-title-area">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          <li class="breadcrumb-item"><a href="{{ url_for('drive') }}">파일</a></li>
          <li class="breadcrumb-item active"><span>공유 관리</span></li>
        </ol>
      </nav>
    </div>
    <div class="drive-nav">
      <a class="nav-pill" href="{{ url_for('change_password') }}">비밀번호 변경</a>
      ...
      <article class="share-row" data-share-id="{{ s.id }}" data-share-url="{{ share_url }}">
        <div class="share-row-main">
          ...
          <div class="share-info">
            ...
            <div class="entry-meta">
              <span class="kind-badge {{ 'folder-badge' if s.kind == 'folder' else 'file-badge' }}">{{ '폴더' if s.kind == 'folder' else kind_label(s.kind, s.name) }}</span>
        ...
        <div class="share-actions">
          <a class="ghost-action" href="{{ share_url }}" target="_blank" rel="noopener">열기</a>
          <button class="ghost-action copy-share" type="button">복사</button>
          <button class="ghost-action danger cancel-share" type="button">공유 취소</button>
        </div>
      </article>
    {% else %}
      <div class="empty-hint share-empty">공유가 없습니다.</div>
    {% endfor %}
  </section>
  <div id="toastHost" class="toast-host"></div>
</div>
...
{% block title %}{% if read_only %}보기{% else %}편집{% endif %} - {{ item.name }}{% endblock %}
{% block body %}
<section class="text-editor-shell" data-save-url="{{ save_url or '' }}" data-read-only="{{ '1' if read_only else '0' }}">
  <header class="text-editor-header">
    ...
    <div class="text-editor-actions">
      <span id="textEditorStatus" class="text-editor-status">{% if read_only %}읽기 전용{% else %}로드됨{% endif %}</span>
      <a class="text-editor-link" href="{{ download_url }}">다운로드</a>
      {% if not read_only %}
        <button id="saveTextButton" class="text-editor-button" type="button">저장</button>
      {% endif %}
    </div>
  </header>
...
// 컨텍스트 메뉴의 미리보기/편집 동작은 파일 목록 페이지를 대체하지 않아야 합니다.
function openInNewTab(url) {
  ...
  showToast('브라우저가 새 탭을 차단했습니다. 팝업을 허용한 뒤 다시 시도하세요.', 4200);
}
...
function api(url, body) {
  return fetch(url, {
    ...
    if (!res.ok || data.ok === false) throw new Error(data.error || '작업에 실패했습니다');
    return data;
  });
}
...
// 폴더 아카이브 생성은 시간이 걸릴 수 있으므로 Flask가 준비된 파일 URL을 반환할 때까지 페이지를 대기시킵니다.
...
async function prepareArchiveDownload(prepareUrl) {
  const overlay = showArchiveWaitingOverlay('압축 중입니다. 잠시만 기다려 주세요...');
  try {
    ...
    if (!response.ok || data.ok === false || !data.url) {
      throw new Error(data.error || '압축에 실패했습니다. 나중에 다시 시도하세요.');
    }
    showToast('압축이 완료되었습니다. 다운로드를 시작합니다.');
...
function openEntryMenu(entry, x, y) {
  ...
  if (kind === 'folder') {
    addMenuItem('열기', () => { window.location.href = `/drive/${id}`; });
    addMenuItem('7z로 다운로드', () => prepareArchiveDownload(`/api/prepare_download/${id}`));
  } else {
    if (mediaKind === 'audio' || mediaKind === 'video') {
      addMenuItem('온라인 재생', () => { openInNewTab(`/preview/${id}`); });
    }
    if (isTextFile) {
      addMenuItem('온라인으로 텍스트 편집', () => { openInNewTab(`/text/${id}`); });
    }
    addMenuItem('다운로드', () => { window.location.href = `/download/${id}`; });
  }

  addMenuItem('이름 변경', async () => {
    const next = prompt('새 이름을 입력하세요:', name);
    if (!next || next.trim() === name) return;
    await api('/api/rename', { item_id: id, name: next.trim() });
    location.reload();
  });
  addMenuItem('공유', async () => {
    const data = await api('/api/share', { item_id: id });
    const copied = await copyText(data.url);
    if (copied) {
      showToast('공유 링크가 클립보드에 복사되었습니다.');
    } else {
      showToast(`복사에 실패했습니다. 직접 복사하세요: ${data.url}`, 8000);
      prompt('공유 링크 복사:', data.url);
    }
  });
  addMenuItem('삭제', async () => {
...
function openBlankMenu(x, y) {
  closeMenu();
  activeEntry = null;
  addMenuItem('파일 업로드', () => fileInput.click());
  addMenuItem('폴더 업로드', () => folderInput.click());
  addMenuItem('새 폴더', async () => {
    const name = prompt('폴더 이름:', '새 폴더');
    if (!name) return;
    await api('/api/folders', { parent_id: currentFolderId, name: name.trim() });
    location.reload();
  });
  addMenuItem('공유 관리', () => { window.location.href = '/shares'; });
  addMenuItem('비밀번호 변경', () => { window.location.href = '/change-password'; });
  addMenuItem('로그아웃', () => { window.location.href = '/logout'; });
  placeMenu(x, y);
}
...
function openInNewTab(url) {
  ...
  alert('브라우저가 새 탭을 차단했습니다. 팝업을 허용한 뒤 다시 시도하세요.');
}
...
async function prepareSharedArchiveDownload(prepareUrl) {
  const overlay = showSharedArchiveWaitingOverlay('압축 중입니다. 잠시만 기다려 주세요...');
  try {
    const response = await fetch(prepareUrl, {
      headers: { 'Accept': 'application/json' },
      credentials: 'same-origin'
...
function openSharedEntryMenu(entryElement, clientX, clientY) {
  ...
  if (entryElement.dataset.kind === 'folder') {
    addSharedMenuItem('열기', () => { window.location.href = entryElement.dataset.openUrl; });
    addSharedMenuItem('7z로 다운로드', () => prepareSharedArchiveDownload(`/api/s/${shareToken}/prepare_archive/${entryElement.dataset.id}`));
  } else {
    addSharedMenuItem('다운로드', () => { window.location.href = entryElement.dataset.downloadUrl; });
    if (entryElement.dataset.playUrl) {
      addSharedMenuItem('온라인 재생', () => { openInNewTab(entryElement.dataset.playUrl); });
    }
    if (entryElement.dataset.textUrl) {
      addSharedMenuItem('온라인으로 텍스트 보기', () => { openInNewTab(entryElement.dataset.textUrl); });
    }
  }
...
document.querySelectorAll('.share-row').forEach(row => {
  row.querySelector('.copy-share')?.addEventListener('click', async () => {
    ...
    if (ok) showToast('공유 링크가 클립보드에 복사되었습니다.');
    else prompt('공유 링크 복사:', url);
  });

  row.querySelector('.cancel-share')?.addEventListener('click', async () => {
    const name = row.querySelector('.entry-name')?.textContent || '이 공유';
    if (!confirm(`"${name}"의 공유를 취소할까요?`)) return;
    const res = await fetch(`/api/shares/${row.dataset.shareId}/cancel`, { method: 'POST' });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.ok === false) {
      showToast(data.error || '공유 취소에 실패했습니다.', 4200);
      return;
    }
    row.remove();
...
async function saveTextDocument() {
  ...
  try {
    ...
    if (!response.ok || responseData.ok === false) throw new Error(responseData.error || '저장에 실패했습니다');
    hasUnsavedTextChanges = false;
    setEditorStatus(`저장됨: ${new Date().toLocaleString()}`);
  } catch (error) {
...
if (textContentEditor) {
  textContentEditor.addEventListener('input', () => {
    ...
    setEditorStatus('저장되지 않은 변경 사항이 있습니다. Ctrl/⌘ + S를 눌러 저장하세요.');
  });
...
def add_security_headers(response):
    """단일 파일 앱에 보수적인 브라우저 보안 헤더를 추가합니다."""
...
def init_db():
    ...
    db.executescript(
        """
        -- 사용자 테이블: 사용자 이름, 비밀번호 솔트, 비밀번호 해시 및 생성 시간을 저장합니다.
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- 사용자 기본 키, 자동 증가 정수.
            username TEXT NOT NULL UNIQUE,        -- 로그인 사용자 이름. 필수이며 전체에서 고유해야 합니다.
            password_salt TEXT NOT NULL,          -- 비밀번호 해싱에 사용되는 사용자별 무작위 솔트.
            password_hash TEXT NOT NULL,          -- 솔트 + 비밀번호로 계산된 SHA3 해시.
            created_at TEXT NOT NULL              -- ISO 문자열로 저장되는 사용자 생성 시간.
        );

        -- 항목 테이블: 파일과 폴더가 하나의 테이블을 공유하며 kind로 구분됩니다.
        CREATE TABLE IF NOT EXISTS entries (
            id INTEGER PRIMARY KEY AUTOINCREMENT,                 -- 항목 기본 키, 자동 증가 정수.
            owner_id INTEGER NOT NULL,                            -- 소유자 사용자 ID. users.id를 참조합니다.
            parent_id INTEGER NULL,                               -- 부모 폴더 ID. NULL은 사용자 루트 폴더를 의미합니다.
            kind TEXT NOT NULL CHECK(kind IN ('file', 'folder')), -- 항목 유형. file 또는 folder만 허용됩니다.
            name TEXT NOT NULL,                                   -- 사용자에게 표시되는 파일 또는 폴더 이름.
            storage_name TEXT NULL,                               -- 디스크에 실제 저장되는 파일 이름. 폴더는 일반적으로 NULL입니다.
            size INTEGER NOT NULL DEFAULT 0,                      -- 파일 크기(바이트). 폴더 기본값은 0입니다.
            created_at TEXT NOT NULL,                             -- ISO 문자열로 저장되는 항목 생성 시간.
            updated_at TEXT NOT NULL,                             -- 항목 업데이트 시간. 이름 변경/이동 작업 시 갱신됩니다.
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE,   -- 사용자가 삭제될 때 항목을 연쇄 삭제합니다.
            FOREIGN KEY(parent_id) REFERENCES entries(id) ON DELETE CASCADE -- 부모 폴더가 삭제될 때 하위 항목을 연쇄 삭제합니다.
        );

        -- 루트 이름 고유성: 한 사용자는 같은 이름의 루트 항목을 두 개 가질 수 없습니다.
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_root
            ON entries(owner_id, name)
            WHERE parent_id IS NULL;

        -- 폴더 이름 고유성: 같은 부모 아래의 항목은 이름을 공유할 수 없습니다.
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_folder
            ON entries(owner_id, parent_id, name)
            WHERE parent_id IS NOT NULL;

        -- 공유 테이블: 공유 토큰을 공유 파일 또는 폴더에 매핑합니다.
        CREATE TABLE IF NOT EXISTS shares (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- 공유 기록 기본 키, 자동 증가 정수.
            token TEXT NOT NULL UNIQUE,           -- 공개 공유 링크에 사용되는 무작위 토큰. 고유해야 합니다.
            item_id INTEGER NOT NULL,             -- 공유 파일 또는 폴더 ID. entries.id를 참조합니다.
            owner_id INTEGER NOT NULL,            -- 공유 생성자 ID. users.id를 참조합니다.
            created_at TEXT NOT NULL,             -- ISO 문자열로 저장되는 공유 생성 시간.
            expires_at TEXT NULL,                 -- 공유 만료 시간. NULL은 현재 만료 시간이 설정되지 않았음을 의미합니다.
            FOREIGN KEY(item_id) REFERENCES entries(id) ON DELETE CASCADE, -- 항목이 삭제될 때 공유를 연쇄 삭제합니다.
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE  -- 사용자가 삭제될 때 공유를 연쇄 삭제합니다.
        );
        """
    )
    db.commit()
...
def cleanup_old_archives():
    """ARCHIVE_RETENTION_SECONDS보다 오래된 생성된 7z 파일을 삭제합니다."""
...
def hash_password(password: str, salt_hex: str | None = None) -> tuple[str, str]:
    """(salt_hex, sha3_256_hash_hex)를 반환합니다. 무작위 솔트와 SHA3-256을 사용합니다."""
    if salt_hex is None:
...
def register():
    if request.method == "POST":
        ...
        # 비밀번호 정책을 의도적으로 느슨하게 유지합니다: 빈 값만 거부합니다.
        if not username or not password:
            flash("사용자 이름과 비밀번호는 비워 둘 수 없습니다.")
...
def change_password():
    ...
    if request.method == "POST":
        ...
        # 여기에서도 동일한 느슨한 규칙을 적용합니다: 새 비밀번호는 비어 있지 않기만 하면 됩니다.
...
def clean_segment(name: str) -> str:
    """유니코드 이름을 유지하고 경로 구분자/제어 문자를 제거하며 빈 이름을 정규화합니다."""
    name = (name or "").replace("\\", "/").split("/")[-1]
    name = "".join(ch for ch in name if ch >= " " and ch not in '<>:"|?*')
    name = name.strip().strip(".")
    return name[:180] or "제목 없음"
...
def unique_name(owner_id: int, parent_id: int | None, desired: str, exclude_id: int | None = None) -> str:
    ...
    for i in range(1, 10000):
        ...
        if not exists(candidate):
            return candidate
    raise RuntimeError("고유한 이름을 생성할 수 없습니다")
...
def create_7z_archive(row) -> Path:
    ...
    with py7zr.SevenZipFile(archive_path, "w", filters=archive_filters()) as archive:
        ...
        if written == 0:
            ...
            marker_path.write_text("이 폴더에는 현재 파일이 없습니다.\n", encoding="utf-8")
...
def api_create_folder():
    ...
    name = clean_segment(data.get("name", "새 폴더"))
...
def api_move():
    ...
    if item["kind"] == "folder" and target_parent_id is not None:
        if int(item["id"]) == target_parent_id or is_descendant(target_parent_id, int(item["id"])):
            return jsonify(ok=False, error="폴더를 자기 자신이나 하위 폴더 안으로 이동할 수 없습니다."), 400
...
def api_cancel_share(share_id):
    ...
    get_db().commit()
    if cur.rowcount == 0:
        return jsonify(ok=False, error="공유가 존재하지 않거나 권한이 없습니다."), 404
    return jsonify(ok=True)
