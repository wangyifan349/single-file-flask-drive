...
=============================================
概览：
    这是一个单文件部署版本。后端路由、HTML 模板、
    CSS 和 JavaScript 都嵌入在 app.py 中。部署时，复制此文件
    并安装所需依赖即可。
主要功能：
    - 登录、注册、退出登录和修改密码。
    - 使用随机盐和 SHA3-256 存储密码摘要。
    - 支持层级文件夹、文件/文件夹上传、列表显示、下载、重命名、
      删除，以及拖放移动。
    - 文件/文件夹分享、分享管理和取消分享。
    - 可浏览共享文件夹；下载文件夹时会生成临时 .7z
      文件，而不会将整个压缩包加载到内存中。
    - 音频/视频在线播放。
    - 纯文本在线编辑；共享页面支持只读文本查看。
    - 读取文本时会检测 BOM 和常见编码；保存时会尝试保留
      原始编码和换行风格。
依赖：
    - flask：Web 服务器、路由、会话和模板渲染。
    - py7zr：创建 .7z 压缩包。
    - charset-normalizer：帮助检测文本文件编码。
安装：
    pip install flask py7zr charset-normalizer
运行：
    python app.py
可选环境变量：
    SECRET_KEY  # 固定会话密钥；生成方式：python -c 'import secrets; print(secrets.token_hex())'
    DRIVE_DATA_DIR
    MAX_CONTENT_LENGTH
...
except ImportError:  # pragma: no cover - 缺少依赖时的运行时回退
    detect_character_sets = None


BASE_DIR = Path(__file__).resolve().parent  # 包含 app.py 的目录
DATA_DIR = Path(os.environ.get("DRIVE_DATA_DIR", BASE_DIR / "data"))  # 数据根目录；可通过环境变量覆盖
UPLOAD_DIR = DATA_DIR / "uploads"  # 上传源文件的目录
TMP_ARCHIVE_DIR = DATA_DIR / "tmp_archives"  # 临时 .7z 压缩包目录
DB_PATH = DATA_DIR / "drive.db"  # SQLite 数据库文件
ARCHIVE_RETENTION_SECONDS = int(os.environ.get("ARCHIVE_RETENTION_SECONDS", 24 * 60 * 60))  # 临时压缩包保留时间

DATA_DIR.mkdir(parents=True, exist_ok=True)  # 首次启动时创建数据目录
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)  # 首次启动时创建上传目录
TMP_ARCHIVE_DIR.mkdir(parents=True, exist_ok=True)  # 首次启动时创建临时压缩包目录

def load_secret_key() -> str:
    """从环境变量或本地数据文件加载稳定的 Flask 密钥。

    部署说明：
        你可以一次性生成固定密钥，并将其设置为 SECRET_KEY：
        python -c 'import secrets; print(secrets.token_hex())'

    保持 SECRET_KEY 稳定，可以避免重启后现有已签名会话失效。
    """
    # 优先使用运维人员提供的密钥，以便生产部署保持固定。
...
app = Flask(__name__, static_folder=None)  # 静态资源由嵌入式路由提供
app.secret_key = load_secret_key()  # 用于签名 Flask 会话
app.config["MAX_CONTENT_LENGTH"] = int(os.environ.get("MAX_CONTENT_LENGTH", 1024 * 1024 * 1024))  # 默认最大上传大小：1 GB
app.config.update(
...
<head>
  ...
  <title>{% block title %}云盘{% endblock %}</title>
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <div class="auth-links"><span></span><a href="{{ url_for('register') }}">注册</a></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <div class="auth-links"><a href="{{ url_for('login') }}">登录</a><span></span></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <div class="auth-links"><a href="{{ url_for('drive') }}">返回</a><a href="{{ url_for('logout') }}">退出登录</a></div>
  </div>
</section>
{% endblock %}
""",
...
<div class="drive-shell" data-current-folder-id="{{ current_folder_id or '' }}">
  ...
  <section id="dropArea" class="file-board" aria-label="文件列表">
      ...
      <div class="entry-card {{ e.kind }}-entry" draggable="true" data-id="{{ e.id }}" data-kind="{{ e.kind }}" data-name="{{ e.name }}" data-media-kind="{{ media_kind(e.name) or '' }}" data-is-text="{{ '1' if is_text_file(e.name) else '0' }}">
        ...
        <div class="entry-main">
          ...
          <div class="entry-meta">
            {% if e.kind == 'folder' %}
              <span class="kind-badge folder-badge">文件夹</span>
            {% else %}
    ...
    {% else %}
      <div class="empty-hint">右键单击空白区域。</div>
    {% endfor %}
  </section>
...
<section class="media-shell">
  <header class="media-header">
    ...
    <a class="mini-download" href="{{ download_url }}">下载</a>
  </header>
...
<section class="media-shell share-single-file" data-share-token="{{ token }}">
  ...
  <section id="sharedFileBoard" class="file-board single-shared-file-board" aria-label="共享文件">
...
<div class="drive-shell shared" data-share-token="{{ token }}">
  <header class="drive-header">
    ...
    <div class="drive-nav">
      <a class="mini-download" data-shared-archive-link="1" data-prepare-url="{{ url_for('share_prepare_archive_download', token=token, folder_id=current_folder_id) }}" href="{{ url_for('share_archive_download', token=token, folder_id=current_folder_id) }}">全部下载为 .7z</a>
    </div>
  </header>

  <section id="sharedFileBoard" class="file-board shared-board" aria-label="共享文件列表">
    {% for e in entries %}
...
<div class="shares-shell">
  <header class="drive-header shares-header">
    <div class="drive-title-area">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          <li class="breadcrumb-item"><a href="{{ url_for('drive') }}">文件</a></li>
          <li class="breadcrumb-item active"><span>分享管理</span></li>
        </ol>
      </nav>
    </div>
    <div class="drive-nav">
      <a class="nav-pill" href="{{ url_for('change_password') }}">修改密码</a>
      <a class="nav-pill" href="{{ url_for('logout') }}">退出登录</a>
    </div>
  </header>
      ...
      <article class="share-row" data-share-id="{{ s.id }}" data-share-url="{{ share_url }}">
        <div class="share-row-main">
          ...
          <div class="share-info">
            ...
            <div class="entry-meta">
              <span class="kind-badge {{ 'folder-badge' if s.kind == 'folder' else 'file-badge' }}">{{ '文件夹' if s.kind == 'folder' else kind_label(s.kind, s.name) }}</span>
        ...
        <div class="share-actions">
          <a class="ghost-action" href="{{ share_url }}" target="_blank" rel="noopener">打开</a>
          <button class="ghost-action copy-share" type="button">复制</button>
          <button class="ghost-action danger cancel-share" type="button">取消分享</button>
        </div>
      </article>
    {% else %}
      <div class="empty-hint share-empty">暂无分享。</div>
    {% endfor %}
  </section>
  <div id="toastHost" class="toast-host"></div>
</div>
...
{% block title %}{% if read_only %}查看{% else %}编辑{% endif %} - {{ item.name }}{% endblock %}
{% block body %}
<section class="text-editor-shell" data-save-url="{{ save_url or '' }}" data-read-only="{{ '1' if read_only else '0' }}">
  <header class="text-editor-header">
    ...
    <div class="text-editor-actions">
      <span id="textEditorStatus" class="text-editor-status">{% if read_only %}只读{% else %}已加载{% endif %}</span>
...
function openInNewTab(url) {
  ...
  showToast('浏览器阻止了新标签页。请允许弹出窗口后重试。', 4200);
}
...
// 文件夹压缩可能需要一些时间；在 Flask 返回已准备好的文件 URL 之前，让页面保持等待状态。
...
async function prepareArchiveDownload(prepareUrl) {
  const overlay = showArchiveWaitingOverlay('正在压缩，请稍候...');
  try {
    ...
    if (!response.ok || data.ok === false || !data.url) {
      throw new Error(data.error || '压缩失败。请稍后重试。');
    }
    showToast('压缩完成。正在开始下载。');
...
function openEntryMenu(entry, x, y) {
  ...
  if (kind === 'folder') {
    ...
    addMenuItem('下载为 7z', () => prepareArchiveDownload(`/api/prepare_download/${id}`));
  } else {
    if (mediaKind === 'audio' || mediaKind === 'video') {
      addMenuItem('在线播放', () => { openInNewTab(`/preview/${id}`); });
    }
    if (isTextFile) {
      addMenuItem('在线编辑文本', () => { openInNewTab(`/text/${id}`); });
    }
    addMenuItem('下载', () => { window.location.href = `/download/${id}`; });
  }

  addMenuItem('重命名', async () => {
    const next = prompt('请输入新名称：', name);
...
function openBlankMenu(x, y) {
  ...
  addMenuItem('分享管理', () => { window.location.href = '/shares'; });
  addMenuItem('修改密码', () => { window.location.href = '/change-password'; });
  addMenuItem('退出登录', () => { window.location.href = '/logout'; });
  placeMenu(x, y);
}
...
async function prepareSharedArchiveDownload(prepareUrl) {
  const overlay = showSharedArchiveWaitingOverlay('正在压缩，请稍候...');
  try {
    const response = await fetch(prepareUrl, {
      headers: { 'Accept': 'application/json' },
      credentials: 'same-origin'
...
function openSharedEntryMenu(entryElement, clientX, clientY) {
  ...
  if (entryElement.dataset.kind === 'folder') {
    ...
    addSharedMenuItem('下载为 7z', () => prepareSharedArchiveDownload(`/api/s/${shareToken}/prepare_archive/${entryElement.dataset.id}`));
  } else {
    addSharedMenuItem('下载', () => { window.location.href = entryElement.dataset.downloadUrl; });
    if (entryElement.dataset.playUrl) {
      addSharedMenuItem('在线播放', () => { openInNewTab(entryElement.dataset.playUrl); });
    }
    if (entryElement.dataset.textUrl) {
      addSharedMenuItem('在线查看文本', () => { openInNewTab(entryElement.dataset.textUrl); });
    }
  }
...
document.querySelectorAll('.share-row').forEach(row => {
  row.querySelector('.copy-share')?.addEventListener('click', async () => {
    ...
    if (ok) showToast('分享链接已复制到剪贴板。');
    else prompt('复制分享链接：', url);
  });

  row.querySelector('.cancel-share')?.addEventListener('click', async () => {
    const name = row.querySelector('.entry-name')?.textContent || '此分享';
    if (!confirm(`取消“${name}”的分享？`)) return;
...
if (textContentEditor) {
  textContentEditor.addEventListener('input', () => {
    ...
    setEditorStatus('有未保存的更改。按 Ctrl/⌘ + S 保存。');
  });
...
def init_db():
    ...
    db.executescript(
        """
        -- 用户表：存储用户名、密码盐、密码哈希和创建时间。
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- 用户主键，自动递增整数。
            username TEXT NOT NULL UNIQUE,        -- 登录用户名；必填且全局唯一。
            password_salt TEXT NOT NULL,          -- 用于密码哈希的用户级随机盐。
            password_hash TEXT NOT NULL,          -- 由盐 + 密码计算得到的 SHA3 哈希。
            created_at TEXT NOT NULL              -- 以 ISO 字符串存储的用户创建时间。
        );

        -- 条目表：文件和文件夹共用一张表，并通过 kind 区分。
        CREATE TABLE IF NOT EXISTS entries (
            id INTEGER PRIMARY KEY AUTOINCREMENT,                 -- 条目主键，自动递增整数。
            owner_id INTEGER NOT NULL,                            -- 所有者用户 ID；引用 users.id。
            parent_id INTEGER NULL,                               -- 父文件夹 ID；NULL 表示用户根文件夹。
            kind TEXT NOT NULL CHECK(kind IN ('file', 'folder')), -- 条目类型；只允许 file 或 folder。
            name TEXT NOT NULL,                                   -- 显示给用户的文件或文件夹名称。
            storage_name TEXT NULL,                               -- 磁盘上的实际存储文件名；文件夹通常为 NULL。
            size INTEGER NOT NULL DEFAULT 0,                      -- 文件大小，单位为字节；文件夹默认为 0。
            created_at TEXT NOT NULL,                             -- 以 ISO 字符串存储的条目创建时间。
            updated_at TEXT NOT NULL,                             -- 条目更新时间；由重命名/移动操作刷新。
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE,   -- 删除用户时级联删除条目。
            FOREIGN KEY(parent_id) REFERENCES entries(id) ON DELETE CASCADE -- 删除父文件夹时级联删除子条目。
        );

        -- 根名称唯一性：同一用户不能拥有两个同名根条目。
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_root
            ON entries(owner_id, name)
            WHERE parent_id IS NULL;

        -- 文件夹内名称唯一性：同一父级下的条目不能同名。
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_folder
            ON entries(owner_id, parent_id, name)
            WHERE parent_id IS NOT NULL;

        -- 分享表：将分享令牌映射到共享文件或文件夹。
        CREATE TABLE IF NOT EXISTS shares (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- 分享记录主键，自动递增整数。
            token TEXT NOT NULL UNIQUE,           -- 公共分享链接中使用的随机令牌；必须唯一。
            item_id INTEGER NOT NULL,             -- 共享文件或文件夹 ID；引用 entries.id。
            owner_id INTEGER NOT NULL,            -- 分享创建者 ID；引用 users.id。
            created_at TEXT NOT NULL,             -- 以 ISO 字符串存储的分享创建时间。
            expires_at TEXT NULL,                 -- 分享过期时间；NULL 表示当前未设置过期时间。
            FOREIGN KEY(item_id) REFERENCES entries(id) ON DELETE CASCADE, -- 删除条目时级联删除分享。
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE  -- 删除用户时级联删除分享。
        );
        """
    )
    db.commit()
...
def cleanup_old_archives():
    """删除早于 ARCHIVE_RETENTION_SECONDS 的已生成 7z 文件。"""
...
def hash_password(password: str, salt_hex: str | None = None) -> tuple[str, str]:
    """返回 (salt_hex, sha3_256_hash_hex)。使用随机盐 + SHA3-256。"""
    if salt_hex is None:
...
def clean_segment(name: str) -> str:
    """保留 Unicode 名称，移除路径分隔符/控制字符，并规范化空名称。"""
...
def create_7z_archive(row) -> Path:
    ...
    with py7zr.SevenZipFile(archive_path, "w", filters=archive_filters()) as archive:
        ...
        if written == 0:
            ...
            marker_path.write_text("此文件夹当前没有文件。\n", encoding="utf-8")
...
def api_move():
    ...
    if item["kind"] == "folder" and target_parent_id is not None:
        if int(item["id"]) == target_parent_id or is_descendant(target_parent_id, int(item["id"])):
            return jsonify(ok=False, error="不能将文件夹移动到自身或其子文件夹中。"), 400
...
def api_cancel_share(share_id):
    ...
    get_db().commit()
    if cur.rowcount == 0:
        return jsonify(ok=False, error="分享不存在，或你没有权限。"), 404
    return jsonify(ok=True)
