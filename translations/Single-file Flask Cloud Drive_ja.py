...
# -*- coding: utf-8 -*-
"""
単一ファイル Flask クラウドドライブ
=============================================
概要:
    これは単一ファイル配布版です。バックエンドルート、HTML テンプレート、
    CSS、JavaScript は app.py に埋め込まれています。デプロイするには、
    このファイルをコピーして必要な依存関係をインストールしてください。
主な機能:
    - ログイン、登録、ログアウト、パスワード変更。
    - ランダムソルトと SHA3-256 で保存されるパスワードダイジェスト。
    - 階層型フォルダー、ファイル/フォルダーのアップロード、一覧表示、ダウンロード、名前変更、
      削除、ドラッグ＆ドロップ移動をサポート。
    - ファイル/フォルダー共有、共有管理、共有解除。
    - 共有フォルダーを閲覧可能。フォルダーダウンロードでは、アーカイブ全体を
      メモリに読み込まず一時 .7z ファイルを生成します。
    - オーディオ/動画のオンライン再生。
    - プレーンテキストのオンライン編集。共有ページでは読み取り専用のテキスト表示をサポート。
    - テキスト読み込みでは BOM と一般的なエンコーディングを検出し、保存時には元の
      エンコーディングと改行スタイルの保持を試みます。
依存関係:
    - flask: Web サーバー、ルート、セッション、テンプレートレンダリング。
    - py7zr: .7z アーカイブを作成。
    - charset-normalizer: テキストファイルのエンコーディング検出を支援。
インストール:
    pip install flask py7zr charset-normalizer
実行:
    python app.py
任意の環境変数:
    SECRET_KEY  # 固定セッションシークレット。生成例: python -c 'import secrets; print(secrets.token_hex())'
    DRIVE_DATA_DIR
    MAX_CONTENT_LENGTH
...
except ImportError:  # pragma: no cover - 依存関係がない場合の実行時フォールバック
    detect_character_sets = None


BASE_DIR = Path(__file__).resolve().parent  # app.py を含むディレクトリ
DATA_DIR = Path(os.environ.get("DRIVE_DATA_DIR", BASE_DIR / "data"))  # データのルートディレクトリ。環境変数で上書き可能
UPLOAD_DIR = DATA_DIR / "uploads"  # アップロードされた元ファイル用ディレクトリ
TMP_ARCHIVE_DIR = DATA_DIR / "tmp_archives"  # 一時 .7z アーカイブディレクトリ
DB_PATH = DATA_DIR / "drive.db"  # SQLite データベースファイル
ARCHIVE_RETENTION_SECONDS = int(os.environ.get("ARCHIVE_RETENTION_SECONDS", 24 * 60 * 60))  # 一時アーカイブの保持時間

DATA_DIR.mkdir(parents=True, exist_ok=True)  # 初回起動時にデータディレクトリを作成
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)  # 初回起動時にアップロードディレクトリを作成
TMP_ARCHIVE_DIR.mkdir(parents=True, exist_ok=True)  # 初回起動時に一時アーカイブディレクトリを作成

def load_secret_key() -> str:
    """環境変数またはローカルデータファイルから安定した Flask シークレットキーを読み込みます。

    デプロイメモ:
        固定キーを一度生成し、SECRET_KEY として設定できます:
        python -c 'import secrets; print(secrets.token_hex())'

    SECRET_KEY を安定させておくと、再起動後に既存の署名済みセッションが無効になるのを防げます。
    """
    # 本番デプロイを固定できるよう、運用者が提供したキーを優先します。
...
app = Flask(__name__, static_folder=None)  # 静的アセットは埋め込みルートで配信されます
app.secret_key = load_secret_key()  # Flask セッションの署名に使用されます
app.config["MAX_CONTENT_LENGTH"] = int(os.environ.get("MAX_CONTENT_LENGTH", 1024 * 1024 * 1024))  # デフォルトの最大アップロードサイズ: 1GB
app.config.update(
...
<head>
  ...
  <title>{% block title %}クラウドドライブ{% endblock %}</title>
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <label class="form-label mt-4">パスワード</label>
      <input class="form-control form-control-lg" name="password" type="password" autocomplete="current-password" required>
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">ログイン</button>
    </form>
    <div class="auth-links"><span></span><a href="{{ url_for('register') }}">登録</a></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    ...
    <form method="post" class="auth-form">
      ...
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">登録</button>
    </form>
    <div class="auth-links"><a href="{{ url_for('login') }}">ログイン</a><span></span></div>
  </div>
</section>
{% endblock %}
""",
...
<section class="auth-layout">
  <div class="auth-panel">
    <h1 class="auth-title">パスワード変更</h1>
    {% for msg in get_flashed_messages() %}<div class="alert alert-warning py-2">{{ msg }}</div>{% endfor %}
    <form method="post" class="auth-form">
      ...
      <label class="form-label mt-4">新しいパスワード</label>
      <input class="form-control form-control-lg" name="new_password" type="password" autocomplete="new-password" required>
      <label class="form-label mt-4">新しいパスワードの確認</label>
      <input class="form-control form-control-lg" name="confirm_password" type="password" autocomplete="new-password" required>
      <button class="btn btn-orange btn-lg w-100 mt-4" type="submit">保存</button>
    </form>
    <div class="auth-links"><a href="{{ url_for('drive') }}">戻る</a><a href="{{ url_for('logout') }}">ログアウト</a></div>
  </div>
</section>
{% endblock %}
""",
    'drive.html': r"""{% extends "base.html" %}
{% block title %}ファイル{% endblock %}
{% block body %}
<div class="drive-shell" data-current-folder-id="{{ current_folder_id or '' }}">
  ...
  <section id="dropArea" class="file-board" aria-label="ファイル一覧">
    {% for e in entries %}
      <div class="entry-card {{ e.kind }}-entry" draggable="true" data-id="{{ e.id }}" data-kind="{{ e.kind }}" data-name="{{ e.name }}" data-media-kind="{{ media_kind(e.name) or '' }}" data-is-text="{{ '1' if is_text_file(e.name) else '0' }}">
        ...
        <div class="entry-main">
          ...
          <div class="entry-meta">
            {% if e.kind == 'folder' %}
              <span class="kind-badge folder-badge">フォルダー</span>
            {% else %}
    ...
    {% else %}
      <div class="empty-hint">空白部分を右クリックしてください。</div>
    {% endfor %}
  </section>
...
{% block title %}オンライン再生 - {{ item.name }}{% endblock %}
{% block body %}
<section class="media-shell">
  <header class="media-header">
    ...
    <a class="mini-download" href="{{ download_url }}">ダウンロード</a>
  </header>
...
<section class="media-shell share-single-file" data-share-token="{{ token }}">
  ...
  <section id="sharedFileBoard" class="file-board single-shared-file-board" aria-label="共有ファイル">
...
<div class="drive-shell shared" data-share-token="{{ token }}">
  <header class="drive-header">
    ...
    <div class="drive-nav">
      <a class="mini-download" data-shared-archive-link="1" data-prepare-url="{{ url_for('share_prepare_archive_download', token=token, folder_id=current_folder_id) }}" href="{{ url_for('share_archive_download', token=token, folder_id=current_folder_id) }}">すべて .7z でダウンロード</a>
    </div>
  </header>

  <section id="sharedFileBoard" class="file-board shared-board" aria-label="共有ファイル一覧">
    {% for e in entries %}
...
<div class="shares-shell">
  <header class="drive-header shares-header">
    <div class="drive-title-area">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
          <li class="breadcrumb-item"><a href="{{ url_for('drive') }}">ファイル</a></li>
          <li class="breadcrumb-item active"><span>共有管理</span></li>
        </ol>
      </nav>
    </div>
    <div class="drive-nav">
      <a class="nav-pill" href="{{ url_for('change_password') }}">パスワード変更</a>
      <a class="nav-pill" href="{{ url_for('logout') }}">ログアウト</a>
    </div>
  </header>
      ...
      <article class="share-row" data-share-id="{{ s.id }}" data-share-url="{{ share_url }}">
        <div class="share-row-main">
          ...
          <div class="share-info">
            ...
            <div class="entry-meta">
              <span class="kind-badge {{ 'folder-badge' if s.kind == 'folder' else 'file-badge' }}">{{ 'フォルダー' if s.kind == 'folder' else kind_label(s.kind, s.name) }}</span>
        ...
        <div class="share-actions">
          <a class="ghost-action" href="{{ share_url }}" target="_blank" rel="noopener">開く</a>
          <button class="ghost-action copy-share" type="button">コピー</button>
          <button class="ghost-action danger cancel-share" type="button">共有を解除</button>
        </div>
      </article>
    {% else %}
      <div class="empty-hint share-empty">共有はありません。</div>
    {% endfor %}
  </section>
  <div id="toastHost" class="toast-host"></div>
</div>
...
{% block title %}{% if read_only %}表示{% else %}編集{% endif %} - {{ item.name }}{% endblock %}
{% block body %}
<section class="text-editor-shell" data-save-url="{{ save_url or '' }}" data-read-only="{{ '1' if read_only else '0' }}">
  <header class="text-editor-header">
    ...
    <div class="text-editor-actions">
      <span id="textEditorStatus" class="text-editor-status">{% if read_only %}読み取り専用{% else %}読み込み済み{% endif %}</span>
      <a class="text-editor-link" href="{{ download_url }}">ダウンロード</a>
      {% if not read_only %}
...
// コンテキストメニューのプレビュー/編集操作は、ファイル一覧ページを置き換えないようにします。
function openInNewTab(url) {
  ...
  showToast('ブラウザーが新しいタブをブロックしました。ポップアップを許可してからもう一度お試しください。', 4200);
}
...
function api(url, body) {
  return fetch(url, {
    ...
    if (!res.ok || data.ok === false) throw new Error(data.error || '操作に失敗しました');
    return data;
  });
}
...
// フォルダーアーカイブの作成には時間がかかる場合があるため、Flask が準備済みファイル URL を返すまでページを待機させます。
...
async function prepareArchiveDownload(prepareUrl) {
  const overlay = showArchiveWaitingOverlay('圧縮中です。しばらくお待ちください...');
  try {
    ...
    if (!response.ok || data.ok === false || !data.url) {
      throw new Error(data.error || '圧縮に失敗しました。後でもう一度お試しください。');
    }
    showToast('圧縮が完了しました。ダウンロードを開始します。');
...
function openEntryMenu(entry, x, y) {
  ...
  if (kind === 'folder') {
    addMenuItem('開く', () => { window.location.href = `/drive/${id}`; });
    addMenuItem('7z でダウンロード', () => prepareArchiveDownload(`/api/prepare_download/${id}`));
  } else {
    if (mediaKind === 'audio' || mediaKind === 'video') {
      addMenuItem('オンライン再生', () => { openInNewTab(`/preview/${id}`); });
    }
    if (isTextFile) {
      addMenuItem('オンラインでテキストを編集', () => { openInNewTab(`/text/${id}`); });
    }
    addMenuItem('ダウンロード', () => { window.location.href = `/download/${id}`; });
  }

  addMenuItem('名前を変更', async () => {
    const next = prompt('新しい名前を入力してください:', name);
    if (!next || next.trim() === name) return;
    await api('/api/rename', { item_id: id, name: next.trim() });
    location.reload();
  });
  addMenuItem('共有', async () => {
    const data = await api('/api/share', { item_id: id });
    const copied = await copyText(data.url);
    if (copied) {
      showToast('共有リンクをクリップボードにコピーしました。');
    } else {
      showToast(`コピーに失敗しました。手動でコピーしてください: ${data.url}`, 8000);
      prompt('共有リンクをコピー:', data.url);
    }
  });
  addMenuItem('削除', async () => {
...
function openBlankMenu(x, y) {
  closeMenu();
  activeEntry = null;
  addMenuItem('ファイルをアップロード', () => fileInput.click());
  addMenuItem('フォルダーをアップロード', () => folderInput.click());
  addMenuItem('新しいフォルダー', async () => {
    const name = prompt('フォルダー名:', '新しいフォルダー');
    if (!name) return;
    await api('/api/folders', { parent_id: currentFolderId, name: name.trim() });
    location.reload();
  });
  addMenuItem('共有管理', () => { window.location.href = '/shares'; });
  addMenuItem('パスワード変更', () => { window.location.href = '/change-password'; });
  addMenuItem('ログアウト', () => { window.location.href = '/logout'; });
  placeMenu(x, y);
}
...
if (board) {
  ...
  async function uploadFiles(fileList, isFolder = false) {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.ok === false) throw new Error(data.error || 'アップロードに失敗しました');
    location.reload();
  }
...
function openInNewTab(url) {
  ...
  alert('ブラウザーが新しいタブをブロックしました。ポップアップを許可してからもう一度お試しください。');
}
...
async function prepareSharedArchiveDownload(prepareUrl) {
  const overlay = showSharedArchiveWaitingOverlay('圧縮中です。しばらくお待ちください...');
  try {
    const response = await fetch(prepareUrl, {
      headers: { 'Accept': 'application/json' },
      credentials: 'same-origin'
...
function openSharedEntryMenu(entryElement, clientX, clientY) {
  ...
  if (entryElement.dataset.kind === 'folder') {
    addSharedMenuItem('開く', () => { window.location.href = entryElement.dataset.openUrl; });
    addSharedMenuItem('7z でダウンロード', () => prepareSharedArchiveDownload(`/api/s/${shareToken}/prepare_archive/${entryElement.dataset.id}`));
  } else {
    addSharedMenuItem('ダウンロード', () => { window.location.href = entryElement.dataset.downloadUrl; });
    if (entryElement.dataset.playUrl) {
      addSharedMenuItem('オンライン再生', () => { openInNewTab(entryElement.dataset.playUrl); });
    }
    if (entryElement.dataset.textUrl) {
      addSharedMenuItem('オンラインでテキストを表示', () => { openInNewTab(entryElement.dataset.textUrl); });
    }
  }
...
document.querySelectorAll('.share-row').forEach(row => {
  row.querySelector('.copy-share')?.addEventListener('click', async () => {
    ...
    if (ok) showToast('共有リンクをクリップボードにコピーしました。');
    else prompt('共有リンクをコピー:', url);
  });

  row.querySelector('.cancel-share')?.addEventListener('click', async () => {
    const name = row.querySelector('.entry-name')?.textContent || 'この共有';
    if (!confirm(`"${name}" の共有を解除しますか？`)) return;
    const res = await fetch(`/api/shares/${row.dataset.shareId}/cancel`, { method: 'POST' });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.ok === false) {
      showToast(data.error || '共有の解除に失敗しました。', 4200);
      return;
    }
    row.remove();
    showToast('共有を解除しました。');
...
async function saveTextDocument() {
  ...
  try {
    ...
    if (!response.ok || responseData.ok === false) throw new Error(responseData.error || '保存に失敗しました');
    hasUnsavedTextChanges = false;
    setEditorStatus(`保存済み: ${new Date().toLocaleString()}`);
  } catch (error) {
...
if (textContentEditor) {
  textContentEditor.addEventListener('input', () => {
    ...
    setEditorStatus('未保存の変更があります。Ctrl/⌘ + S を押して保存してください。');
  });
...
def add_security_headers(response):
    """単一ファイルアプリに保守的なブラウザーセキュリティヘッダーを追加します。"""
...
def init_db():
    ...
    db.executescript(
        """
        -- ユーザーテーブル: ユーザー名、パスワードソルト、パスワードハッシュ、作成時刻を保存します。
        CREATE TABLE IF NOT EXISTS users (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- ユーザー主キー、自動増分整数。
            username TEXT NOT NULL UNIQUE,        -- ログインユーザー名。必須で全体で一意です。
            password_salt TEXT NOT NULL,          -- パスワードハッシュ化に使用するユーザーごとのランダムソルト。
            password_hash TEXT NOT NULL,          -- ソルト + パスワードから計算された SHA3 ハッシュ。
            created_at TEXT NOT NULL              -- ISO 文字列として保存されるユーザー作成時刻。
        );

        -- エントリーテーブル: ファイルとフォルダーが 1 つのテーブルを共有し、kind で区別されます。
        CREATE TABLE IF NOT EXISTS entries (
            id INTEGER PRIMARY KEY AUTOINCREMENT,                 -- エントリー主キー、自動増分整数。
            owner_id INTEGER NOT NULL,                            -- 所有者ユーザー ID。users.id を参照します。
            parent_id INTEGER NULL,                               -- 親フォルダー ID。NULL はユーザーのルートフォルダーを意味します。
            kind TEXT NOT NULL CHECK(kind IN ('file', 'folder')), -- エントリー種別。file または folder のみ許可されます。
            name TEXT NOT NULL,                                   -- ユーザーに表示されるファイルまたはフォルダー名。
            storage_name TEXT NULL,                               -- ディスク上の実際の保存ファイル名。フォルダーは通常 NULL です。
            size INTEGER NOT NULL DEFAULT 0,                      -- ファイルサイズ（バイト）。フォルダーの既定値は 0 です。
            created_at TEXT NOT NULL,                             -- ISO 文字列として保存されるエントリー作成時刻。
            updated_at TEXT NOT NULL,                             -- エントリー更新時刻。名前変更/移動操作で更新されます。
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE,   -- ユーザーが削除されたときにエントリーをカスケード削除します。
            FOREIGN KEY(parent_id) REFERENCES entries(id) ON DELETE CASCADE -- 親フォルダーが削除されたときに子エントリーをカスケード削除します。
        );

        -- ルート名の一意性: 1 人のユーザーは同じ名前のルートエントリーを 2 つ持てません。
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_root
            ON entries(owner_id, name)
            WHERE parent_id IS NULL;

        -- フォルダー名の一意性: 同じ親の下にあるエントリーは名前を共有できません。
        CREATE UNIQUE INDEX IF NOT EXISTS entries_unique_name_folder
            ON entries(owner_id, parent_id, name)
            WHERE parent_id IS NOT NULL;

        -- 共有テーブル: 共有トークンを共有ファイルまたはフォルダーに対応付けます。
        CREATE TABLE IF NOT EXISTS shares (
            id INTEGER PRIMARY KEY AUTOINCREMENT, -- 共有レコード主キー、自動増分整数。
            token TEXT NOT NULL UNIQUE,           -- 公開共有リンクで使用するランダムトークン。一意である必要があります。
            item_id INTEGER NOT NULL,             -- 共有ファイルまたはフォルダー ID。entries.id を参照します。
            owner_id INTEGER NOT NULL,            -- 共有作成者 ID。users.id を参照します。
            created_at TEXT NOT NULL,             -- ISO 文字列として保存される共有作成時刻。
            expires_at TEXT NULL,                 -- 共有有効期限。NULL は現在有効期限が設定されていないことを意味します。
            FOREIGN KEY(item_id) REFERENCES entries(id) ON DELETE CASCADE, -- エントリーが削除されたときに共有をカスケード削除します。
            FOREIGN KEY(owner_id) REFERENCES users(id) ON DELETE CASCADE  -- ユーザーが削除されたときに共有をカスケード削除します。
        );
        """
    )
    db.commit()
...
def cleanup_old_archives():
    """ARCHIVE_RETENTION_SECONDS より古い生成済み 7z ファイルを削除します。"""
...
def hash_password(password: str, salt_hex: str | None = None) -> tuple[str, str]:
    """(salt_hex, sha3_256_hash_hex) を返します。ランダムソルト + SHA3-256 を使用します。"""
    if salt_hex is None:
...
def register():
    if request.method == "POST":
        ...
        # パスワードポリシーは意図的に緩くしています: 空の値のみ拒否します。
        if not username or not password:
            flash("ユーザー名とパスワードは空にできません。")
...
def change_password():
    ...
    if request.method == "POST":
        ...
        # ここでも同じ緩いルールを適用します: 新しいパスワードは空でなければ十分です。
...
def clean_segment(name: str) -> str:
    """Unicode 名を保持し、パス区切り文字/制御文字を削除し、空の名前を正規化します。"""
    name = (name or "").replace("\\", "/").split("/")[-1]
    name = "".join(ch for ch in name if ch >= " " and ch not in '<>:"|?*')
    name = name.strip().strip(".")
    return name[:180] or "無題"
...
def unique_name(owner_id: int, parent_id: int | None, desired: str, exclude_id: int | None = None) -> str:
    ...
    for i in range(1, 10000):
        ...
        if not exists(candidate):
            return candidate
    raise RuntimeError("一意の名前を生成できません")
...
def create_7z_archive(row) -> Path:
    ...
    with py7zr.SevenZipFile(archive_path, "w", filters=archive_filters()) as archive:
        ...
        if written == 0:
            ...
            marker_path.write_text("このフォルダーには現在ファイルがありません。\n", encoding="utf-8")
...
def api_create_folder():
    ...
    name = clean_segment(data.get("name", "新しいフォルダー"))
...
def api_move():
    ...
    if item["kind"] == "folder" and target_parent_id is not None:
        if int(item["id"]) == target_parent_id or is_descendant(target_parent_id, int(item["id"])):
            return jsonify(ok=False, error="フォルダーを自分自身またはそのサブフォルダーへ移動することはできません。"), 400
...
def api_cancel_share(share_id):
    ...
    get_db().commit()
    if cur.rowcount == 0:
        return jsonify(ok=False, error="共有が存在しないか、権限がありません。"), 404
    return jsonify(ok=True)
